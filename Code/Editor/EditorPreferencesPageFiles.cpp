...
void CEditorPreferencesPage_Files::Reflect(AZ::SerializeContext& serialize)
{
    ...
    if (editContext)
    {
        ...
        editContext->Class<ExternalEditors>("External Editors", "External Editors")
            ->DataElement(AZ::Edit::UIHandlers::ExeSelectBrowseEdit, &ExternalEditors::m_scripts, "Scripts Editor", "Scripts Text Editor (Default to Dusk Engine internal tool when empty)")
            ->Attribute(AZ::Edit::Attributes::PlaceholderText, "Default to Dusk Engine internal tool when empty")
