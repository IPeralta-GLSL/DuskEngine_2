...
namespace AzQtComponents
{
        ...
        , m_tabContainerIdentifierPrefix(QString("_%1tabcontainer_").arg(identifierPrefix))
    {
        ...
        QObject::connect(m_linuxDragTimer, &QTimer::timeout, this, &FancyDocking::onLinuxDragPoll);
#endif
...
#if defined(AZ_PLATFORM_LINUX)
    void FancyDocking::onLinuxDragPoll()
    {
        if (!m_dropZoneState.dragging() || !m_state.dock)
        {
            m_linuxDragTimer->stop();
            return;
        }
        ...
        if (!(buttons & Qt::LeftButton))
        {
            ...
            dropDockWidget(m_state.dock, m_dropZoneState.dropOnto(), area);
            return;
        }

        QPoint localPos = m_mainWindow->mapFromGlobal(globalPos);
        QMouseEvent moveEvent(QEvent::MouseMove, localPos, globalPos, Qt::LeftButton, buttons, QApplication::keyboardModifiers());
        dockMouseMoveEvent(m_state.dock, &moveEvent);
    }
#endif
