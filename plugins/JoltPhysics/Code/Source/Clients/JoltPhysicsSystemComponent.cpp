...
namespace JoltPhysics
{
    ...
    bool JoltPhysicsSystemComponent::CookConvexMeshToMemory(const AZ::Vector3* vertices, AZ::u32 vertexCount,
        AZStd::vector<AZ::u8>& result)
    {
        result.resize(sizeof(AZ::u32) + sizeof(JPH::Float3) * vertexCount);

        AZ::u8* dataPtr = result.data();
        memcpy(dataPtr, &vertexCount, sizeof(AZ::u32));
        dataPtr += sizeof(AZ::u32);

        for (AZ::u32 vertexIndex = 0; vertexIndex < vertexCount; ++vertexIndex)
        {
            JPH::Float3 float3 = { vertices[vertexIndex].GetX(), vertices[vertexIndex].GetY(), vertices[vertexIndex].GetZ() };
            memcpy(dataPtr, &float3, sizeof(JPH::Float3));
    ...
    bool JoltPhysicsSystemComponent::CookTriangleMeshToMemory(const AZ::Vector3* vertices, AZ::u32 vertexCount,
        const AZ::u32* indices, AZ::u32 indexCount, AZStd::vector<AZ::u8>& result)
    {
        result.resize(sizeof(AZ::u32) * 2 + sizeof(JPH::Float3) * vertexCount + sizeof(AZ::u32) * indexCount);
        ...
        memcpy(dataPtr, &indexCount, sizeof(AZ::u32));
        dataPtr += sizeof(AZ::u32);

        memcpy(dataPtr, indices, sizeof(AZ::u32) * indexCount);
