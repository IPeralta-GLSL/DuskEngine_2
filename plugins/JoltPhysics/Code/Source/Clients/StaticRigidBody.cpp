...
namespace JoltPhysics
{
    ...
    StaticRigidBody::~StaticRigidBody()
    {
        ...
        if (m_joltStaticBody)
        {
            ...
            auto& bodyInterface = m_owningSystem->GetBodyInterface();
            if (bodyInterface.IsAdded(m_joltStaticBody->GetID()))
            {
                bodyInterface.RemoveBody(m_joltStaticBody->GetID());
            }
            bodyInterface.DestroyBody(m_joltStaticBody->GetID());
            m_joltStaticBody = nullptr;
        }
    }
    ...
    void StaticRigidBody::AddShape(AZStd::shared_ptr<Physics::Shape> shape)
    {
        ...
        if (joltShape && joltShape->GetNativePointer())
        {
            ...
            const JPH::Shape* nativeShape = static_cast<const JPH::Shape*>(joltShape->GetNativePointer());
            AZ_TracePrintf("JoltStaticRigidBody", "AddShape %s: type=%d", m_debugName.c_str(),
                nativeShape ? static_cast<AZ::u32>(nativeShape->GetType()) : 0);
        }
        else
        {
