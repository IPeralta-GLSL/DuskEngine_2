...
namespace JoltPhysics
{
    namespace Utils
    {
            ...
            AZStd::vector<const JoltPhysicsMaterial*>& inMaterials)
        {
            ...
            case Physics::ShapeType::CookedMesh:
                {
                    const auto& cookedConfig = dynamic_cast<const Physics::CookedMeshShapeConfiguration&>(shapeConfiguration);
                    const AZStd::vector<AZ::u8>& cookedData = cookedConfig.GetCookedMeshData();
                    if (cookedData.empty())
                    {
                        AZ_Error("Jolt Utils", false, "Cooked mesh shape configuration has no cooked data.")
                        return false;
                    }
                    ...
                    {
                        AZ::u32 pointCount = 0;
                        memcpy(&pointCount, dataPtr, sizeof(AZ::u32));
                        dataPtr += sizeof(AZ::u32);

                        AZStd::vector<JPH::Vec3> points;
                        points.reserve(pointCount);
                        for (AZ::u32 pointIndex = 0; pointIndex < pointCount; ++pointIndex)
                        {
                            JPH::Float3 float3;
                        ...
                        JPH::ConvexHullShapeSettings settings(
                            points.data(), aznumeric_cast<int>(points.size()), JPH::cDefaultConvexRadius, material);
                        if (material)
                        {
                    ...
                    else
                    {
                        ...
                        memcpy(&vertexCount, dataPtr, sizeof(AZ::u32));
                        dataPtr += sizeof(AZ::u32);
                        ...
                        AZ::u32 indexCount = 0;
                        memcpy(&indexCount, dataPtr, sizeof(AZ::u32));
                        dataPtr += sizeof(AZ::u32);

                        JPH::IndexedTriangleList triangles;
                        triangles.reserve(indexCount / 3);
                        for (AZ::u32 index = 0; index < indexCount; index += 3)
                        {
                            AZ::u32 indices[3];
                            memcpy(indices, dataPtr, sizeof(AZ::u32) * 3);
                            dataPtr += sizeof(AZ::u32) * 3;
                            triangles.push_back(JPH::IndexedTriangle(indices[0], indices[1], indices[2], 0));
                        }
                                     ...
                                     const JPH::BroadPhaseLayer& broadPhaseLayer)
        {
            ...
            {
                AZ::u32 newBPLayer = static_cast<AZ::u8>(broadPhaseLayer) + 1;
