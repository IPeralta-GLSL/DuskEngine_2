...
namespace JoltPhysics
{
	...
	public:
		...
		[[nodiscard]] JPH::BroadPhaseLayer GetBroadPhaseLayer(JPH::ObjectLayer inLayer) const override
		{
			...
			return JPH::BroadPhaseLayer(static_cast<AZ::u8>(inLayer) - 1);
		}
	...
	public:
		...
		{
			// BroadPhase is stored as (index + 1) in ObjectLayer, with 0 reserved as invalid
			const AZ::u8 mask1 = GetBroadPhaseCollisionMask(static_cast<JoltBroadPhaseLayer>(static_cast<AZ::u8>(inLayer1) - 1));
			const AZ::u8 mask2 = GetBroadPhaseCollisionMask(static_cast<JoltBroadPhaseLayer>(static_cast<AZ::u8>(inLayer2)));

			const bool collide = (mask1 & mask2) != 0;

			static int s_bpFilterLogCount = 0;
			if (s_bpFilterLogCount++ < 40)
			{
				AZ_TracePrintf("JoltFilter", "BpFilter obj=0x%08x bpLayer=%d mask1=%d mask2=%d -> %s",
					inLayer1, static_cast<AZ::u8>(inLayer2), mask1, mask2,
					collide ? "COLLIDE" : "NO");
			}
