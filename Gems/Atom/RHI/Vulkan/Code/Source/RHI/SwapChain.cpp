...
namespace AZ
{
    namespace Vulkan
    {
            ...
            if (m_pendingRecreation)
            {
                ...
                if (oldSwapchain != VK_NULL_HANDLE)
                {
                    ...
                    device.QueueForRelease(
                        new ReleaseContainer<VkSwapchainKHR>(
                            device.GetNativeDevice(),
                            oldSwapchain,
                            device.GetContext().DestroySwapchainKHR));
                }
                InitImages();
        ...
        RHI::ResultCode SwapChain::ResizeInternal(const RHI::SwapChainDimensions& dimensions, RHI::SwapChainDimensions* nativeDimensions)
        {
            ...
            clampedDimensions.m_imageWidth = AZStd::max(clampedDimensions.m_imageWidth, 1u);
            clampedDimensions.m_imageHeight = AZStd::max(clampedDimensions.m_imageHeight, 1u);
            m_dimensions = clampedDimensions;
